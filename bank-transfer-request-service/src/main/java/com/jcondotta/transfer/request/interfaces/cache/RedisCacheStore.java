package com.jcondotta.transfer.request.interfaces.cache;
...
public class RedisCacheStore<V> implements CacheStore<V> {
    ...
    @Override
    public Optional<V> computeIfAbsentWithLock(String cacheKey, Supplier<Optional<V>> mapping) {
        ...
        try {
            ...
            if (!acquired) {
                // outra instância já está computando: pequena espera e lê novamente
                Thread.sleep(50);
                return getIfPresent(cacheKey);
            }

            // segunda verificação depois do lock
            cachedValue = getIfPresent(cacheKey);
            if (cachedValue.isPresent()) return cachedValue;

            Optional<V> computed = mapping.get();
            computed.ifPresent(v -> put(cacheKey, v));
            return computed;
